...
void GGXDistribution::GenerateSampleVector(const Vector2& r, const Vector3& wo, const Vector3& normal, const PbrMaterial& material, Vector3& wi) const
{
...
float GGXDistribution::Sample(const Vector3& wo, const Vector3& wi, const Vector3& normal, const PbrMaterial& material) const
{
	Vector3 h = VectorUtil<3>::Normalize(wo + wi);

	float alpha2 = powf(material.roughness, 4);
...
float GGXDistribution::CalculatePDF(const Vector3& wo, const Vector3& wi, const Vector3& normal, const PbrMaterial& material)
{
	float alpha2 = pow(material.roughness, 4);
